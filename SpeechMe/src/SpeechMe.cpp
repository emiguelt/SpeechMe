...
void SpeechMe::loadSettings(){
    conf = new Configuration();
    QSettings settings("USP","SpeechMe");
    conf->setServerPort(settings.value("serverPort", 10000).toInt());
    conf->setConfigFile(settings.value("configFile", "").toString().toStdString());
}

void SpeechMe::writeSettings(){
    QSettings settings("USP","SpeechMe");
